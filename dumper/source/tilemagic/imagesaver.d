...
void saveImage(T, Arr, P, size_t N, size_t N2)(string path, const T[] tiles, const Arr tilemap, const P[N][N2] palettes, bool firstColourTransparent = true) {
	saveImage(path, tiles, cast(Arrangement)tilemap, convertPalettes(palettes, firstColourTransparent));
}
...
void saveImage(T, Arr, P)(string path, const T[] tiles, const Arr tilemap, const P[][] palettes, bool firstColourTransparent = true) if (!is(Arr == Arrangement)) {
	saveImage(path, tiles, cast(Arrangement)tilemap, convertPalettes(palettes, firstColourTransparent));
}
void saveImage(T, P, size_t N, size_t N2)(string path, const T[] tiles, const P[N][N2] palettes, size_t width, bool firstColourTransparent = true, ArrangementStyle style = ArrangementStyle.horizontal) {
	saveImage(path, tiles, Arrangement.generate(style, tiles.length, width), convertPalettes(palettes, firstColourTransparent));
}
...
void saveImage(Tile)(string path, const Tile[] tiles, const Arrangement tilemap, const RGBA8888[][] palettes) @system
	in(tilemap.width > 0)
	in(palettes.length > 0)
{
	...
	foreach (tileidx, tileattrs; tilemap.tiles) {
		...
		const baseX = (tileidx / width) * tiles[0].width;
		const baseY = (tileidx % width) * tiles[0].height;
