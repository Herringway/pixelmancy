...
version(Posix)
package(justimages) void makeNonBlocking(int fd) {
...
version(Posix)
package(justimages) void setCloExec(int fd) {
	import core.sys.posix.fcntl;
...
enum ThreadToRunIn {
	...
	/++
		The callback will be duplicated across all threads known to the justimages.core event loop.
...
/++
	Initializes the arsd core event loop and creates its worker threads. You don't actually have to call this, since the first use of an justimages.core function that requires it will call it implicitly, but calling it yourself gives you a chance to control the configuration more explicitly if you want to.
+/
...
+/
package(justimages) enum EventLoopType {
	/++
...
version(HasThread) package(justimages) class CallbackHelper {
	import core.memory;
...
version(HasFile) class AsyncFile : AbstractFile {
	...
	package(justimages) this(NativeFileHandle adoptPreSetup) {
		super(adoptPreSetup);
	}
...
struct UserProvidedBuffer(T) {
	...
	package(justimages) bool append(T item) {
	...
	package(justimages) T[] slice() return {
...
/++
	...
	History:
		previously in minigui as a private function. Moved to justimages.core on April 3, 2023
+/
...
package(justimages) int indexOf(scope const(char)[] haystack, scope const(char)[] needle) {
...
package(justimages) int indexOf(scope const(ubyte)[] haystack, scope const(char)[] needle) {
...
package(justimages) bool endsWith(scope const(char)[] haystack, scope const(char)[] needle) {
...
package(justimages) bool startsWith(scope const(char)[] haystack, scope const(char)[] needle) {
	if(needle.length > haystack.length)
		return false;
...
private class CoreEventLoopImplementation : ICoreEventLoop {
	...
	version(Posix) {
		...
		/++
			Informs the justimages.core system that the given signal happened. You can call this from inside a signal handler.
		+/
...
STDIO
	...
	/++
		Please note using this will create a compile-time dependency on [justimages.terminal]
...
Stdout can represent either
	...
	/++
		...
		It works correctly on Windows and is user friendly on Linux (using justimages.terminal.getline)
		while also working if stdin has been redirected (where justimages.terminal itself would throw)
...
package(justimages) version(Windows) extern(Windows) {
...
package(justimages) version(OSXCocoa) {
